...
function MovieFind() {
    ...
    const [mtitle,setMtitle] = useState("all");
    const {isLoading,isError,error,data,refetch:movieFindData}=useQuery(
        ["movie_find",curpage],
        async ()=>{
            return await apiClient.get(`/movie/find/${curpage}/${mtitle}`)
        }
    )
    const find=(e)=>{
                    ...
                    <div className="row">
                        <div className="col-lg-12">
                            <div className="section-heading">
                                <h2>영화 검색</h2>
                                <input type={"text"} size={"20px"} className={"input-sm"} onChange={find} value={mtitle} />
